...
# Function to add random noise to sleep duration (±30%)
random_sleep() {
  base=$1
  variance=$(echo "$base * 0.3" | bc)
  noise=$(echo "scale=2; ($RANDOM / 32767) * 2 * $variance - $variance" | bc)
  duration=$(echo "scale=0; ($base + $noise) / 1" | bc)
